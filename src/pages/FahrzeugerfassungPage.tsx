...
export default function FahrzeugerfassungPage() {
  const navigate = useNavigate();
  const [records, setRecords] = useState<Fahrzeugerfassung[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Fahrzeugerfassung | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Fahrzeugerfassung | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, fahrerverwaltungData] = await Promise.all([
        LivingAppsService.getFahrzeugerfassung(),
  ...
  async function handleCreate(fields: Fahrzeugerfassung['fields']) {
    await LivingAppsService.createFahrzeugerfassungEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Fahrzeugerfassung['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateFahrzeugerfassungEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteFahrzeugerfassungEntry(deleteTarget.record_id);
  ...
  function getFahrerverwaltungDisplayName(url?: unknown) {
    ...
    return fahrerverwaltungList.find(r => r.record_id === id)?.fields.vorname ?? '—';
  }
  ...
  return (
    <PageShell
      title="Fahrzeugerfassung"
      subtitle={`${records.length} Fahrzeugerfassung im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Fahrzeugerfassung suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/fahrzeugerfassung/${record.record_id}`); }}>
                <TableCell className="font-medium">{record.fields.kennzeichen ?? '—'}</TableCell>
                <TableCell>{record.fields.fahrzeugbezeichnung ?? '—'}</TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={7} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Fahrzeugerfassung. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <ConfirmDialog
        ...
        title="Fahrzeugerfassung löschen"
