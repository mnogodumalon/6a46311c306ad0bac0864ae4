...
import type { Fahrzeugerfassung, Fahrerverwaltung } from '@/types/app';
import { LivingAppsService, extractRecordId, cleanFieldsForApi } from '@/services/livingAppsService';
import { FahrzeugerfassungDialog } from '@/components/dialogs/FahrzeugerfassungDialog';
import { FahrzeugerfassungViewDialog } from '@/components/dialogs/FahrzeugerfassungViewDialog';
...
const FAHRZEUGERFASSUNG_FIELDS = [
  { key: 'kennzeichen', label: 'Kennzeichen', type: 'string/text' },
  { key: 'fahrzeugbezeichnung', label: 'Fahrzeugbezeichnung', type: 'string/text' },
  { key: 'fahrer', label: 'Fahrer', type: 'applookup/select', targetEntity: 'fahrerverwaltung', targetAppId: 'FAHRERVERWALTUNG', displayField: 'vorname' },
  { key: 'standort', label: 'Aktueller Standort', type: 'geo' },
  { key: 'status', label: 'Status', type: 'lookup/radio', options: [{ key: 'im_einsatz', label: 'Im Einsatz' }, { key: 'in_wartung', label: 'In Wartung' }, { key: 'verfuegbar', label: 'Verfügbar' }, { key: 'ausser_betrieb', label: 'Außer Betrieb' }] },
...
const ENTITY_TABS = [
  { key: 'fahrzeugerfassung', label: 'Fahrzeugerfassung', pascal: 'Fahrzeugerfassung' },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('fahrzeugerfassung');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'fahrzeugerfassung': new Set(),
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'fahrzeugerfassung': return (data as any).fahrzeugerfassung as Fahrzeugerfassung[] ?? [];
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'fahrzeugerfassung' && fieldKey === 'fahrer') {
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'fahrzeugerfassung': return FAHRZEUGERFASSUNG_FIELDS;
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'fahrzeugerfassung': return {
        create: (fields: any) => LivingAppsService.createFahrzeugerfassungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateFahrzeugerfassungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteFahrzeugerfassungEntry(id),
      };
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'fahrzeugerfassung' || dialogState?.entity === 'fahrzeugerfassung') && (
        <FahrzeugerfassungDialog
          open={createEntity === 'fahrzeugerfassung' || dialogState?.entity === 'fahrzeugerfassung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'fahrzeugerfassung' ? handleUpdate : (fields: any) => handleCreate('fahrzeugerfassung', fields)}
          defaultValues={dialogState?.entity === 'fahrzeugerfassung' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'fahrzeugerfassung' && (
        <FahrzeugerfassungViewDialog
          open={viewState?.entity === 'fahrzeugerfassung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'fahrzeugerfassung', record: r }); }}
          fahrerverwaltungList={(data as any).fahrerverwaltung ?? []}
        />
      )}
