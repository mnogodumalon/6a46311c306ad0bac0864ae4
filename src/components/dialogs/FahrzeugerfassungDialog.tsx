...
import type { Fahrzeugerfassung, Fahrerverwaltung, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Fahrzeugerfassung';
...
interface FahrzeugerfassungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Fahrzeugerfassung['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<Fahrzeugerfassung['fields'], 'status'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  status: LOOKUP_OPTIONS['fahrzeugerfassung']?.['status'] ?? [],
};
...
export function FahrzeugerfassungDialog({ open, onClose, onSubmit, defaultValues, recordId, fahrerverwaltungList, enablePhotoScan = true, enablePhotoLocation = true }: FahrzeugerfassungDialogProps) {
  const [fields, setFields] = useState<Partial<Fahrzeugerfassung['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['kennzeichen', 'status'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Fahrzeugerfassung['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'fahrzeugerfassung');
      await onSubmit(clean as Fahrzeugerfassung['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "kennzeichen": string | null, // Kennzeichen\n  "fahrzeugbezeichnung": string | null, // Fahrzeugbezeichnung\n  "fahrer": string | null, // Display name from Fahrerverwaltung (see <available-records>)\n  "status": LookupValue | null, // Status (select one key: "im_einsatz" | "in_wartung" | "verfuegbar" | "ausser_betrieb") mapping: im_einsatz=Im Einsatz, in_wartung=In Wartung, verfuegbar=Verfügbar, ausser_betrieb=Außer Betrieb\n  "bemerkung": string | null, // Bemerkung\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["fahrer"]);
        ...
        if (fahrerName) {
          const fahrerMatch = fahrerverwaltungList.find(r => matchName(fahrerName!, [[r.fields.vorname ?? '', r.fields.nachname ?? ''].filter(Boolean).join(' ')]));
          if (fahrerMatch) merged['fahrer'] = createRecordUrl(APP_IDS.FAHRERVERWALTUNG, fahrerMatch.record_id);
        }
        return merged as Partial<Fahrzeugerfassung['fields']>;
      });
      if (gps) {
  ...
  const DIALOG_INTENT = defaultValues ? 'Fahrzeugerfassung bearbeiten' : 'Fahrzeugerfassung hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'fahrzeugbezeichnung': (
      <div key="fahrzeugbezeichnung" className="space-y-1.5">
    ...
    'standort': (
      <div key="standort" className="space-y-1.5">
        <Label htmlFor="standort">Aktueller Standort</Label>
      ...
      <div key="status" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'im_einsatz'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'im_einsatz' ? undefined : 'im_einsatz') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'im_einsatz'
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'ausser_betrieb'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'ausser_betrieb' ? undefined : 'ausser_betrieb') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'ausser_betrieb'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'bemerkung': (
      <div key="bemerkung" className="space-y-1.5">
        ...
        <Textarea
          id="bemerkung"
          placeholder="Besonderheiten, Wartungshinweise, Anmerkungen..."
  ...
  const FIELD_LABELS: Record<string, string> = {"kennzeichen": "Kennzeichen", "fahrzeugbezeichnung": "Fahrzeugbezeichnung", "fahrer": "Fahrer", "standort": "Aktueller Standort", "status": "Status", "bemerkung": "Bemerkung"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"fahrer": {"vorname": "Vorname", "nachname": "Nachname", "telefon": "Telefonnummer", "email": "E-Mail-Adresse"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.FAHRZEUGERFASSUNG} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
    ...
    {createFahrerverwaltungOpen && (
      <FahrerverwaltungDialog
        ...
        defaultValues={createFahrerverwaltungInitial
          ? ({ vorname: createFahrerverwaltungInitial } as any)
          : undefined}
      />
    )}
    </>
