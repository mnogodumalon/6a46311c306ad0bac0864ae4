...
import type { Fahrzeugerfassung, Fahrerverwaltung, CreateFahrzeugerfassung, CreateFahrerverwaltung } from '@/types/app';
...
export class LivingAppsService {
  // --- FAHRZEUGERFASSUNG ---
  static async getFahrzeugerfassung(): Promise<Fahrzeugerfassung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.FAHRZEUGERFASSUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Fahrzeugerfassung[];
    return enrichLookupFields(records, 'fahrzeugerfassung');
  }
  static async getFahrzeugerfassungEntry(id: string): Promise<Fahrzeugerfassung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.FAHRZEUGERFASSUNG}/records/${id}`);
    const record = { record_id: data.id, ...data } as Fahrzeugerfassung;
    return enrichLookupFields([record], 'fahrzeugerfassung')[0];
  }
  static async createFahrzeugerfassungEntry(fields: CreateFahrzeugerfassung) {
    return callApi('POST', `/apps/${APP_IDS.FAHRZEUGERFASSUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'fahrzeugerfassung') });
  }
  static async updateFahrzeugerfassungEntry(id: string, fields: Partial<CreateFahrzeugerfassung>) {
    return callApi('PATCH', `/apps/${APP_IDS.FAHRZEUGERFASSUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'fahrzeugerfassung') });
  }
  static async deleteFahrzeugerfassungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.FAHRZEUGERFASSUNG}/records/${id}`);
  }
  ...
  static async getFahrerverwaltungEntry(id: string): Promise<Fahrerverwaltung | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Fahrerverwaltung;
    return enrichLookupFields([record], 'fahrerverwaltung')[0];
  }
  static async createFahrerverwaltungEntry(fields: CreateFahrerverwaltung) {
    return callApi('POST', `/apps/${APP_IDS.FAHRERVERWALTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'fahrerverwaltung') });
  }
  static async updateFahrerverwaltungEntry(id: string, fields: Partial<CreateFahrerverwaltung>) {
    return callApi('PATCH', `/apps/${APP_IDS.FAHRERVERWALTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'fahrerverwaltung') });
  }
